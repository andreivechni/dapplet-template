    ...
    if (this.insPointName === 'MENU') {
      this.el.innerHTML = `
        <div style="padding: 9px 12px 10px 12px;
          ${isActive ? 'border-bottom: 2px solid #4285f4; ' : 'border-bottom: none; '}
          display: block; cursor: pointer;"
          ${tooltip ? `title="${tooltip}"` : ''}
        >
          <img style="height: 14px; width: 14px; vertical-align: top;" src="${img}"/>
          <div style="display: inline-block; font-size: 13px; line-hight: 16px; ${isActive ? 'color: #4285f4; font-weight: bold;' : '-webkit-tap-highlight-color: rgba(0,0,0,.10); color: #5f6368; font-weight: normal;'}">${label}</div>
        </div>
      `;
      activeNavEl.style.borderBottom = isActive ? 'none' : '2px solid #4285f4';
    } else if (
      ...
      this.el.innerHTML = `
        <div 
          ...
          ${tooltip ? `title="${tooltip}"` : ''}
        >
          <img style="width: 20px; margin-right: 12px; margin-bottom: 3px;" src="${img}"/>
